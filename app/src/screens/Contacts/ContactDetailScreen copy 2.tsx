...
// ✨ IMPROVEMENT: Removed unused icons for cleaner code
import { ArrowLeft, Star, MoreVertical, Phone, MessageSquare, IndianRupee, Briefcase, Users, MapPin, Trash2, Edit, Share2, Shield, Archive, Book } from 'lucide-react-native';
import tw from 'twrnc';
import { useLocalSearchParams, useRouter } from 'expo-router';
import { LinearGradient } from 'expo-linear-gradient';
import { formatRelative } from 'date-fns'; // ✨ FIX: This import is necessary for formatting dates

// --- Assuming these are your real context hooks ---
import { useTheme } from '../../context/ThemeContext';
import { useLanguage } from '../../context/LanguageContext';
// --- Assuming this is your calendar component ---
import { AttendanceCalendar } from '../../components/ui/AttendanceCalendar';
// --- Importing all data and types from the data file ---
...
// ✨ FEATURE: Added a new dashboard to show details for Business contacts
const BusinessDashboard: FC<{ contact: Contact; t: any }> = ({ contact, t }) => {
    if (!contact.businessDetails) return null;
    return (
        <InfoCard title={t.businessInfo}>
            <DetailRow icon={Briefcase} label={t.labels.category} value={contact.businessDetails.category} />
            <DetailRow icon={MapPin} label={t.labels.location} value={contact.businessDetails.location} />
            <DetailRow icon={Book} label={t.labels.services} value={contact.businessDetails.services.join(', ')} />
        </InfoCard>
    );
};
...
export default function ContactDetailScreen() {
    ...
    // ✨ ROBUSTNESS: Added a fallback to 'en' to prevent crashes with unknown locales
    ...
    if (!contact) {
        return (
            <SafeAreaView style={[tw`flex-1 items-center justify-center`, { backgroundColor: theme.colors.background }]}>
                <Text style={{ color: theme.colors.text }}>Contact not found.</Text>
            </SafeAreaView>
        );
    }
        ...
        <SafeAreaView style={[tw`flex-1 mb-20`, { backgroundColor: theme.colors.background }]}>
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
                ...
                <View style={tw`-mt-14 items-center`}>
                    ...
                    <Text style={[tw`text-2xl font-bold mt-2`, { color: theme.colors.text }]}>{contact.name}</Text>
                    <View style={tw`flex-row items-center mt-4`}>
                        <Text style={[tw`text-base`, { color: theme.colors.textSecondary }]}>{contact.phone}</Text>
                        <View style={[tw`ml-3 `]}><Text style={[tw`text-xs font-bold px-3 py-1 capitalize rounded-full`, roleTagStyle]}>{contact.role}</Text></View>
                    </View>
                </View>
                ...
                {contact.role === 'business' && <BusinessDashboard contact={contact} t={t} />}

                {contact.interactionHistory && (
                    <InfoCard title={t.history}>
                        {contact.interactionHistory.map((item, index) => (
                             // ✨ CRITICAL FIX: The Date object MUST be formatted to a string before rendering.
                            <DetailRow 
                                key={index} 
                                icon={item.type === 'call' ? Phone : MessageSquare} 
                                label={formatRelative(item.time, new Date())} 
                                value={item.text || 'Call made'} 
                            />
                        ))}
                    </InfoCard>
                )}
